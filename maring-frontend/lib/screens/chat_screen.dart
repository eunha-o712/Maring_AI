...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  void initState() {
    super.initState();
    _conversationIdFuture = ref.read(sessionProvider.notifier).ensureConversation();
  }
      ...
      body: FutureBuilder<String>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError || !snapshot.hasData) {
            return Center(child: Text('대화를 시작하지 못했어요.\n${snapshot.error ?? ''}'));
          }
...
class _ChatBodyState extends ConsumerState<_ChatBody> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(chatProvider(widget.conversationId), (previous, next) {
      if (next.lastSafety != null) {
        showSafetyOverlay(context, next.lastSafety!).then((_) {
          ref.read(chatProvider(widget.conversationId).notifier).dismissSafetyOverlay();
        });
      }
    });
